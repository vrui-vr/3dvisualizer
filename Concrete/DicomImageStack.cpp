...
Visualization::Abstract::DataSet* DicomImageStack::load(const std::vector<std::string>& args,Cluster::MulticastPipe* pipe) const
	...
	if(fileName.empty())
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No DICOM file name provided");
	...
	if(getBaseDirectory()->getPathType(fileName.c_str())==Misc::PATHTYPE_DIRECTORY)
		{
		/* Open the directory: */
		IO::DirectoryPtr directory=getBaseDirectory()->openDirectory(fileName.c_str());
		...
		if(!isd.isValid())
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Directory %s does not contain a valid image series",fileName.c_str());
		}
	else
		{
		/* Open the DICOM directory file: */
		DicomFile dcmDirectory(fileName.c_str(),openFile(fileName));
		...
		if(!isd.isValid())
			{
			if(seriesNumber==-1)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Directory file %s does not contain a valid image series",fileName.c_str());
			else
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Directory file %s does not contain a valid image series %d",fileName.c_str(),seriesNumber);
			}
		}
	...
	for(int i=0;i<isd->numImages;++i)
		...
		DicomFile dcm(isd->imageFileNames[i],openFile(isd->imageFileNames[i]));
