...
Visualization::Abstract::DataSet* StructuredHexahedralTecplotASCIIFile::load(const std::vector<std::string>& args,Cluster::MulticastPipe* pipe) const
	...
	while(true)
		...
		if(parser.getZoneType()!=TecplotASCIIFileHeaderParser::STRUCTURED)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"File %s has unstructured zones",dataFileName);
		if(parser.getZoneLayout()!=TecplotASCIIFileHeaderParser::INTERLEAVED)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"File %s has unsupported zone layout",dataFileName);
			...
			for(index[1]=0;index[1]<numZoneVertices[1];++index[1])
				for(index[2]=0;index[2]<numZoneVertices[2];++index[2],++line)
					...
					try
						...
						{
						throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot read zone from file %s at vertex (%d, %d, %d) in line %u due to exception %s",dataFileName,index[0],index[1],index[2],(unsigned int)line,err.what());
						}
