...
readVectorAttributes(
	...
	size_t size=dataSet.getTotalNumVertices();
	size_t index=0;
	for(size_t percentRead=1;percentRead<=100;++percentRead)
		{
		size_t indexEnd=(size*percentRead+50)/100;
		for(;index<indexEnd;++index)
...
Visualization::Abstract::DataSet* UnstructuredHexahedralVTK::load(const std::vector<std::string>& args,Cluster::MulticastPipe* pipe) const
	...
	if(headerSource.readString()!="#"||headerSource.readString()!="vtk"||headerSource.readString()!="DataFile"||headerSource.readString()!="Version")
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Input file %s is not a VTK data file",args[0].c_str());
	...
	if(vtkVersionMajor>3||(vtkVersionMajor==3&&vtkVersionMinor>0))
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s is unsupported version %d.%d",args[0].c_str(),vtkVersionMajor,vtkVersionMinor);
	...
	if(headerSource.readChar()!='\n')
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s has malformed storage type definition",args[0].c_str());
	if(storageType=="BINARY")
		binary=true;
	else if(storageType!="ASCII")
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s has unrecognized storage type %s",args[0].c_str(),storageType.c_str());
	
	/* Read the data set descriptor: */
	if(headerSource.readString()!="DATASET")
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s does not have a data set definition",args[0].c_str());
	std::string dataSetType=headerSource.readString();
	if(dataSetType!="UNSTRUCTURED_GRID")
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s has data set type %s instead of UNSTRUCTURED_GRID",args[0].c_str(),dataSetType.c_str());
	if(headerSource.readChar()!='\n')
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s has malformed data set definition",args[0].c_str());
	
	/* Read the grid point data type: */
	if(headerSource.readString()!="POINTS")
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s does not define grid points",args[0].c_str());
	numGridPoints=headerSource.readInteger();
	gridPointDataType=headerSource.readString();
	if(headerSource.getChar()!='\n')
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s has malformed grid point definition",args[0].c_str());
	}
	...
	if(binary)
		...
		if(gridPointDataType=="unsigned_char")
			...
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unsupported grid point data type %s",gridPointDataType.c_str());
		}
	else
		{
		...
		for(int percentRead=1;percentRead<=100;++percentRead)
			...
			for(;index<indexEnd;++index)
				...
				if(gridSource.getChar()!='\n')
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid vertex position in VTK data file %s",args[0].c_str());
				dataSet.addVertex(vertexPosition);
				}
	...
	if(headerSource.readString()!="CELLS")
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s does not define grid cells",args[0].c_str());
	numGridCells=headerSource.readInteger();
	DS::CellIndex numNumbers=headerSource.readInteger();
	if(numNumbers!=numGridCells*(1+8))
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s is not a hexahedral grid",args[0].c_str());
	if(headerSource.getChar()!='\n')
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s has malformed grid point definition",args[0].c_str());
	}
	...
	if(binary)
		...
		for(int percentRead=1;percentRead<=100;++percentRead)
			...
			for(;index<indexEnd;++index)
				...
				file->read(indices,9);
				if(indices[0]!=8)
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Non-hexahedral grid cell in VTK data file %s",args[0].c_str());
				...
				if(gridSource.getChar()!='\n')
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid grid cell in VTK data file %s",args[0].c_str());
	...
	if(headerSource.readString()!="CELL_TYPES")
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s does not define grid cell types",args[0].c_str());
	if(headerSource.readInteger()!=numGridCells)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s has mismatching number of grid cell types",args[0].c_str());
	}
		...
		for(int percentRead=1;percentRead<=100;++percentRead)
			...
			for(;index<indexEnd;++index)
				...
				if(gridSource.readInteger()!=12) // VTK code for hexahedra
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Non-hexahedral grid cell in VTK data file %s",args[0].c_str());
				if(gridSource.getChar()!='\n')
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid grid cell type in VTK data file %s",args[0].c_str());
				}
	...
	while(true)
		...
		if(attributeSource.readInteger()!=numGridPoints)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s defines wrong number of point attributes",args[0].c_str());
		if(attributeSource.readChar()!='\n')
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s has malformed point attribute definition",args[0].c_str());
		...
		if(attributeType=="SCALARS")
			...
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s has unknown point attribute type %s",args[0].c_str(),attributeType.c_str());
		...
		if(binary)
			...
			if(attributeVectors)
				...
				if(attributeScalarType=="unsigned_char")
					...
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unsupported attribute scalar data type %s in vector attribute %s",attributeScalarType.c_str(),attributeName.c_str());
				}
			else
				{
				if(attributeScalarType=="unsigned_char")
					...
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unsupported attribute scalar data type %s in scalar attribute %s",attributeScalarType.c_str(),attributeName.c_str());
				}
			}
		else
			{
				...
				if(attributeVectors)
					...
					for(index[1]=0;index[1]<numVertices[1];++index[1])
						for(index[0]=0;index[0]<numVertices[0];++index[0])
							...
							if(attributeSource.getChar()!='\n')
								throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid vector attribute in in VTK data file %s",args[0].c_str());
