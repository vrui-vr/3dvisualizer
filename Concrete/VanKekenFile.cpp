...
Visualization::Abstract::DataSet* VanKekenFile::load(const std::vector<std::string>& args,Cluster::MulticastPipe* pipe) const
	...
	if(master)
		...
		if(numGrids!=dataNumGrids)
			...
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Grid file %s and data file %s have mismatching sizes",args[0].c_str(),args[1].c_str());
			}
		else if(pipe!=0)
			{
			pipe->write<int>(1);
		...
		for(int gridIndex=0;gridIndex<numGrids;++gridIndex)
			...
			DS::Index gridSize;
			gridFile.read(gridSize.getComponents(),3);
			DS::Index dataGridSize;
			dataFile.read(dataGridSize.getComponents(),3);
			if(gridSize!=dataGridSize)
				...
				{
				pipe->write<int>(1);
				pipe->write(gridSize.getComponents(),3);
				}
			...
			if(pipe!=0)
				...
				for(DS::Array::iterator vIt=vertices.begin();vIt!=vertices.end();++vIt)
					{
					gridFile.read(vIt->pos.getComponents(),3);
					pipe->write(vIt->pos.getComponents(),3);
					dataFile.read(vIt->value.scalar);
					pipe->write(vIt->value.scalar);
					dataFile.read(vIt->value.vector.getComponents(),3);
					pipe->write(vIt->value.vector.getComponents(),3);
					}
				}
			else
				{
		...
		for(int gridIndex=0;gridIndex<numGrids;++gridIndex)
			...
			DS::Index gridSize;
			pipe->read(gridSize.getComponents(),3);
			...
			for(DS::Array::iterator vIt=vertices.begin();vIt!=vertices.end();++vIt)
				{
				pipe->read(vIt->pos.getComponents(),3);
				pipe->read(vIt->value.scalar);
				pipe->read(vIt->value.vector.getComponents(),3);
				}
			}
...
Visualization::Abstract::DataSet* VanKekenFile::load(const std::vector<std::string>& args,Cluster::MulticastPipe* pipe) const
	...
	if(coordFiles.size()!=valueFiles.size())
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Mismatching numbers of coordinate and value files");
	int numGrids=coordFiles.size();
	for(int i=0;i<numGrids;++i)
		if(coordFiles[i].gridIndex!=i)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing coordinate file for grid index %d",i);
	for(int i=0;i<numGrids;++i)
		if(valueFiles[i].gridIndex!=i)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing value file for grid index %d",i);
	...
	for(int gridIndex=0;gridIndex<numGrids;++gridIndex)
		...
		for(int i=0;i<gridSize[1]*gridSize[0];++i)
			...
			if(fscanf(coordFile.getFilePtr(),"%lf %lf",&longLats[i].latitude,&longLats[i].longitude)!=2)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Premature end of file in coordinate file %s",coordFileName);
		...
		for(int i=0;i<gridSize[2];++i)
			...
			if(radius!=gridRadii[i])
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Mismatching grid radius in coordinate file %s",coordFileName);
			}
		...
		if(fscanf(valueFile.getFilePtr(),"%d %d",&dummy,&numValues)!=2)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing header in value file %s",valueFileName);
		if(numValues!=gridSize[0]*gridSize[1]*gridSize[2])
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Mismatching number of values in value file %s",valueFileName);
			...
			for(vIndex[1]=0;vIndex[1]<gridSize[1];++vIndex[1])
				...
				for(vIndex[2]=gridSize[2]-1;vIndex[2]>=0;--vIndex[2])
					...
					if(fscanf(valueFile.getFilePtr(),"%lf %lf %lf %lf",&vector[0],&vector[1],&vector[2],&scalar)!=4)
						throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Premature end of file in value file %s",valueFileName);
	...
	for(int gridIndex=0;gridIndex<numGrids;++gridIndex)
		...
		gridFile.write(gridSize.getComponents(),3);
		dataFile.write(gridSize.getComponents(),3);
		...
		for(DS::Array::const_iterator vIt=vertices.begin();vIt!=vertices.end();++vIt)
			{
			gridFile.write(vIt->pos.getComponents(),3);
			dataFile.write(vIt->value.scalar);
			dataFile.write(vIt->value.vector.getComponents(),3);
			}
		}
	#endif
