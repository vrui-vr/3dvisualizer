...
Visualization::Abstract::DataSet* CitcomCUSphericalRawFile::load(const std::vector<std::string>& args,Cluster::MulticastPipe* pipe) const
	...
	{
	IO::ValueSource headerSource(openFile((*argIt)+".hdr"));
	...
	if(gridType=="multigrid")
		...
		try
			...
			{
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid multigrid definition in header file %s",((*argIt)+".hdr").c_str());
			}
		}
		...
		try
			...
			{
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid conjugate gradient definition in header file %s",((*argIt)+".hdr").c_str());
			}
		}
	else
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unrecognized mesh type %s in header file %s",gridType.c_str(),((*argIt)+".hdr").c_str());
	...
	try
		...
		{
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid number of CPUs in header file %s",((*argIt)+".hdr").c_str());
		}
	}
	...
	for(DS::Index cpuIndex(0);cpuIndex[0]<numCpus[0];cpuIndex.preInc(numCpus),++cpuCounter)
		...
		for(int i=0;i<3;++i)
			...
			IO::FilePtr gridFile(openFile(gridFileName));
	...
	for(++argIt;argIt!=args.end();++argIt)
		...
		if(strcasecmp(argIt->c_str(),"-log")==0)
			...
			for(DS::Index cpuIndex(0);cpuIndex[0]<numCpus[0];cpuIndex.preInc(numCpus),++cpuCounter)
				...
				IO::FilePtr dataFile(openFile(dataFileName));
