...
Copyright (c) 2008-2018-2024 Oliver Kreylos
...
Visualization::Abstract::DataSet* CitcomSRegionalASCIIFile::load(const std::vector<std::string>& args,Cluster::MulticastPipe* pipe) const
	...
	IO::FilePtr cfgFile(openFile(*argIt));
	std::string dataDirName;
	std::string dataFileName;
	int numSurfaces=0;
	DS::Index numCpus(0,0,0);
	DS::Index numVertices(0,0,0);
	parseCitcomSCfgFile(*argIt,cfgFile,dataDirName,dataFileName,numSurfaces,numCpus,numVertices);
	if(numSurfaces==0||numCpus.calcIncrement(-1)==0||numVertices.calcIncrement(-1)==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"%s is not a valid CitcomS configuration file",argIt->c_str());
	
	/* Check if it's really a regional model: */
	if(numSurfaces!=1)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Configuration file %s does not describe a regional model; use CitcomSGlobalASCIIFile instead",argIt->c_str());
	
	/* Open the data directory, which is relative to the configuration file's location: */
	IO::DirectoryPtr dataDir=getBaseDirectory()->openFileDirectory(argIt->c_str());
	dataDir=dataDir->openDirectory(dataDirName.c_str());
	...
	for(DS::Index cpuIndex(0);cpuIndex[0]<numCpus[0];cpuIndex.preInc(numCpus),++cpuCounter)
		...
		std::string coordFileName=dataFileName;
		coordFileName.append(".coord.");
		coordFileName.append(Misc::ValueCoder<int>::encode(cpuLinearIndex));
		IO::ValueSource coordReader(dataDir->openFile(coordFileName.c_str()));
		coordReader.skipWs();
		...
		try
			...
			if(coordReader.readInteger()!=totalCpuNumVertices)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Mismatching grid size in coordinate file %s",coordFileName.c_str());
			}
		catch(const IO::ValueSource::NumberError&)
			{
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid header line in coordinate file %s",coordFileName.c_str());
			}
			...
			for(gridIndex[0]=0;gridIndex[0]<cpuNumVertices[0];++gridIndex[0])
				for(gridIndex[2]=0;gridIndex[2]<cpuNumVertices[2];++gridIndex[2])
					...
					try
						...
						{
						throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid vertex definition in coordinate file %s",coordFileName.c_str());
						}
					}
		if(master)
	...
	if(!isNumber)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No time step index provided");
	...
	for(++argIt;argIt!=args.end();++argIt)
		...
		if(*argIt=="-log")
			...
			for(DS::Index cpuIndex(0);cpuIndex[0]<numCpus[0];cpuIndex.preInc(numCpus),++cpuCounter)
				...
				int cpuLinearIndex=(cpuIndex[1]*numCpus[0]+cpuIndex[0])*numCpus[2]+cpuIndex[2];
				std::string dataValueFileName=dataFileName;
				...
				IO::ValueSource dataValueReader(dataDir->openFile(dataValueFileName.c_str()));
				...
				try
					...
					if(dataValueFileNumVertices1!=totalCpuNumVertices||dataValueFileNumVertices2!=totalCpuNumVertices)
						throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Mismatching grid size in data value file %s",dataValueFileName.c_str());
					}
				catch(const IO::ValueSource::NumberError&)
					{
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid header line in data value file %s",dataValueFileName.c_str());
					}
					...
					for(gridIndex[0]=0;gridIndex[0]<cpuNumVertices[0];++gridIndex[0])
						for(gridIndex[2]=0;gridIndex[2]<cpuNumVertices[2];++gridIndex[2])
							...
							try
								...
								{
								throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid vertex value definition in data value file %s",dataValueFileName.c_str());
								}
							}
				if(master)
					std::cout<<"\b\b\b\b"<<std::setw(3)<<((cpuCounter+1)*100)/numCpus.calcIncrement(-1)<<"%"<<std::flush;
