...
Visualization::Abstract::DataSet* MultiCitcomtFile::load(const std::vector<std::string>& args,Cluster::MulticastPipe* pipe) const
	...
	*********************************************************/
	...
	for(int i=0;i<numVars;++i)
		if(dataColumn[i]<0)
			{
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Data variable %s not found in CITCOMT header in input file %s",varStart[i],args[0].c_str());
			headerValid=false;
			}
	if(numFreeVariables>0)
		{
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Too few data variables in CITCOMT header in input file %s",args[0].c_str());
		headerValid=false;
		}
	if(!headerValid)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid CITCOMT header in input file %s",args[0].c_str());
