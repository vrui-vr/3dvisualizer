...
Visualization::Abstract::DataSet* SeismicTomographyModel::load(const std::vector<std::string>& args,Cluster::MulticastPipe* pipe) const
	...
	for(unsigned int i=0;i<args.size();++i)
		...
		if(args[i][0]=='-')
			...
			if(strcasecmp(args[i].c_str()+1,"size")==0) // Size is given in lat, long, depth order
				...
				if(j<4)
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Too few components in -column option");
				int columnMask=0x0;
				for(int j=0;j<4;++j)
					columnMask|=0x1<<column[j];
				if(columnMask!=0xf)
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"-column option does not define a permutation");
				}
				...
				if(j<3)
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Too few components in -order option");
				int orderMask=0x0;
				for(int j=0;j<3;++j)
					orderMask|=0x1<<order[j];
				if(orderMask!=0x7)
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"-order option does not define a permutation");
				}
	...
	if(!haveNumVertices)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing data set size");
	if(dataFileName==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing data set file name");
	...
	if(multiFile)
		...
		if(!hasFloatConversion)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Slice file name template \"%s\" does not contain %f float conversion",dataFileName);
		if(numConversions>1)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Slice file name template \"%s\" contains too many conversions",dataFileName);
		...
		if(sliceFileDir==0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Could not open slice file directory %s",sliceBaseDir.c_str());
