...
Visualization::Abstract::DataSet* UnstructuredHexahedralTecplotASCIIFile::load(const std::vector<std::string>& args,Cluster::MulticastPipe* pipe) const
	...
	for(std::vector<std::string>::const_iterator argIt=args.begin();argIt!=args.end();++argIt)
		...
		if((*argIt)[0]=='-')
			...
			if(strcasecmp(argIt->c_str()+1,"coords")==0)
				...
				--argIt;
				if(i<3)
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing coordinate name on command line");
				}
				...
				++argIt;
				if(argIt==args.end())
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing vector variable name on command line");
				...
				--argIt;
				if(i<3)
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing vector component name on command line");
				}
			}
	...
	if(dataFileName==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No input file name provided");
	if(scalarNames.empty()&&vectorNames.empty())
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No scalar or vector variables specified");
	
	/* Create a parser and open the input file: */
	TecplotASCIIFileHeaderParser parser(openFile(dataFileName));
	...
	for(int i=0;i<3;++i)
		...
		if(variableIndex>=parser.getNumVariables())
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing vertex coordinate %s in file %s",coordNames[i],dataFileName);
	...
	for(int i=0;i<numScalars;++i)
		...
		for(variableIndex=0;variableIndex<parser.getNumVariables();++variableIndex)
			if(strcasecmp(parser.getVariableName(variableIndex).c_str(),scalarNames[i].c_str())==0)
				break;
		if(variableIndex>=parser.getNumVariables())
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing scalar variable %s in file %s",scalarNames[i].c_str(),dataFileName);
		...
		for(int j=0;j<3;++j)
			...
			for(variableIndex=0;variableIndex<parser.getNumVariables();++variableIndex)
				if(strcasecmp(parser.getVariableName(variableIndex).c_str(),vectorComponentNames[i*3+j].c_str())==0)
					break;
			if(variableIndex>=parser.getNumVariables())
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing vector variable component %s in file %s",vectorComponentNames[i*3+j].c_str(),dataFileName);
	...
	while(true)
		...
		if(parser.getZoneType()!=TecplotASCIIFileHeaderParser::UNSTRUCTURED)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"File %s has structured zones");
		if(parser.getZoneElementType()!=TecplotASCIIFileHeaderParser::HEXAHEDRON)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"File %s has unsupported element type");
		if(parser.getZoneLayout()!=TecplotASCIIFileHeaderParser::INTERLEAVED)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"File %s has unsupported zone layout");
		...
		for(int i=0;i<parser.getZoneNumVertices();++i)
			...
			try
				...
				{
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot read zone vertices from file %s due to exception %s",dataFileName,err.what());
				}
		...
		for(int i=0;i<parser.getZoneNumElements();++i)
			...
			try
				...
				{
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Cannot read zone cells from file %s due to exception %s",dataFileName,err.what());
				}
