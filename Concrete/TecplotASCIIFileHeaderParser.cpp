...
void TecplotASCIIFileHeaderParser::parseZone(void)
	...
	while(!eof()&&peekc()!='\n')
		...
		/* Check for the equal sign: */
		if(!readEqual())
			throw Misc::makeStdErr(0,"Missing = in ZONE header");
		...
		if(tag.length()==1)
			...
			if(t=='T')
				...
				if(strcasecmp(vPtr,"POINT")==0)
					...
					throw Misc::makeStdErr(0,"Invalid format specifier %s",value.c_str());
				}
			else if(t=='N')
				{
				zoneType=UNSTRUCTURED;
			...
			if(t0=='E'&&t1=='T')
				...
				if(value=="BRICK")
					...
					throw Misc::makeStdErr(0,"Unsupported element type %s",value.c_str());
				}
			}
	...
	if(zoneType==STRUCTURED)
		...
		if(zoneSize[0]<0||zoneSize[1]<0||zoneSize[2]<0)
			throw Misc::makeStdErr(0,"Missing structured zone size specification");
		}
	else
		{
		if(zoneElementType==INVALID)
			throw Misc::makeStdErr(0,"Missing element type specification");
		if(zoneNumVertices<0||zoneNumElements<0)
			throw Misc::makeStdErr(0,"Missing unstructured zone size specification");
		}
	}
...
TecplotASCIIFileHeaderParser::TecplotASCIIFileHeaderParser(IO::FilePtr source)
	...
	if(!readNextZoneHeader())
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Malformed header in input file");
	}
	...
	while(!eof())
		...
		if(peekc()!='#'&&peekc()!='\n')
			...
			if(keyword=="TITLE")
				...
				/* Read the equal sign: */
				if(!readEqual())
					throw Misc::makeStdErr(0,"Missing = after TITLE keyword");
				...
				/* Read the equal sign: */
				if(!readEqual())
					throw Misc::makeStdErr(0,"Missing = after VARIABLES keyword");
