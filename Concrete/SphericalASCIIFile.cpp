...
Visualization::Abstract::DataSet* SphericalASCIIFile::load(const std::vector<std::string>& args,Cluster::MulticastPipe* pipe) const
		...
		if((*argIt)[0]=='-')
			...
			if(strcasecmp(argIt->c_str()+1,"headers")==0)
				...
				for(int i=0;i<3;++i)
					...
					if(speed<0||speed>=3)
						throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid node counting speed %d specified, must be 0, 1, or 2",speed);
	...
	if(dataFileName==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No data file name provided");
	if(numVertices.calcIncrement(-1)==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No number of nodes provided");
	if(numDataSlices==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No scalar or vector data values specified");
	
	/* Open the data file: */
	IO::ValueSource reader(openFile(dataFileName));
		...
		for(index[nodeCountOrder[1]]=0;index[nodeCountOrder[1]]<numVertices[nodeCountOrder[1]];++index[nodeCountOrder[1]])
			for(index[nodeCountOrder[2]]=0;index[nodeCountOrder[2]]<numVertices[nodeCountOrder[2]];++index[nodeCountOrder[2]])
				...
				try
					...
					{
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Number format error in line %u",lineNumber);
					}
