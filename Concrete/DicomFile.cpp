...
class MemoryImageWriter:public JPEGImageWriter // Class to write image data into a memory buffer
	...
	virtual void setImageParameters(const int newImageSize[2],int numScanComponents,int numBits)
		...
		if(newImageSize[0]!=imageSize[0]||newImageSize[1]!=imageSize[1]||numScanComponents!=1)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Mismatching image parameters");
		};
...
#if 0 // Not actually used at the moment
...
DicomFile::DicomFile(const char* dcmFileName,IO::SeekableFilePtr sDcmFile)
	...
	if(strncasecmp(prefix,"DICM",4)!=0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"File \"%s\" is not a DICOM file",dcmFileName);
...
DicomFile::ImageDescriptor* DicomFile::readImageDescriptor(void)
	...
	if(fileType!=FILE_IMAGE)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"File is not an image file");
	...
	while(true)
		...
		if(de.is(0x0018U,0x0050U)) // Slice thickness
			...
			{
			if(sequenceTop==0)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Out-of-context sequence item");
			...
			{
			if(sequenceTop==0)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Out-of-context end-of-sequence element");
...
class DicomImageFileSorter
	...
	/* Embedded classes: */
	public:
	enum SortMode // Enumerated type for image sorting modes
		{
		BySliceIndex,BySlicePos,ByName
		};
	
	/* Elements: */
	private:
	SortMode sortMode; // The sorting mode
	
	/* Constructors and destructors: */
	public:
	DicomImageFileSorter(SortMode sSortMode)
		:sortMode(sSortMode)
		{
		}
	...
	bool operator()(const DicomImageFile& dif1,const DicomImageFile& dif2)
		...
		switch(sortMode)
			...
			case BySlicePos:
				return dif1.second->imagePos[2]<dif2.second->imagePos[2];
			
			case ByName:
				return dif1.first<dif2.first;
			
			default:
				return false; // Never reached; just to make compiler happy
			}
		}
	};
...
DicomFile::ImageStackDescriptor* DicomFile::readImageStackDescriptor(IO::DirectoryPtr directory)
	{
	/* Read all DICOM image files in the directory and collect their image and pixel sizes and image positions: */
	std::vector<DicomImageFile> images;
	int stackImageSize[2]={0,0};
	float stackPixelSize[3]={0.0f,0.0f,0.0f};
	float stackImagePos[3]={0.0f,0.0f,0.0f};
	bool notFirst=false;
	bool stackConsistent=true;
		...
		try
			...
			if(directory->getEntryType()==Misc::PATHTYPE_FILE)
				...
				if(id!=0)
					...
					if(notFirst)
						{
						/* Compare the image's size and pixel size against the stack: */
						stackConsistent=stackConsistent&&id->imageSize[0]==stackImageSize[0]&&id->imageSize[1]==stackImageSize[1];
						stackConsistent=stackConsistent&&id->pixelSize[0]==stackPixelSize[0]&&id->pixelSize[1]==stackPixelSize[1]&&id->sliceThickness==stackPixelSize[2];
						
						/* Compare the image's position against the stack: */
						stackConsistent=stackConsistent&&id->imagePos[0]==stackImagePos[0]&&id->imagePos[1]==stackImagePos[1];
						...
						{
						/* Initialize the stack image size and pixel size: */
						for(int i=0;i<2;++i)
							stackImageSize[i]=id->imageSize[i];
						for(int i=0;i<2;++i)
							stackPixelSize[i]=id->pixelSize[i];
						stackPixelSize[2]=id->sliceThickness;
						
						/* Initialize the stack image position: */
						for(int i=0;i<2;++i)
							stackImagePos[i]=id->imagePos[i];
	...
	if(images.empty())
		{
		std::cerr<<"DicomFile::readImageStackDescriptor: No slice images found"<<std::endl;
		return 0;
		}
	
	/* Check if the image stack has consistent image and pixel sizes: */
	if(!stackConsistent)
		{
		std::cerr<<"DicomFile::readImageStackDescriptor: Image stack has inconsistent image or pixel sizes or image positions"<<std::endl;
		return 0;
		}
	
	/* Sort the image files by slice index: */
	DicomImageFileSorter difs(DicomImageFileSorter::BySliceIndex);
	std::sort(images.begin(),images.end(),difs);
	
	/* Check if the image stack's slice indices are consecutive: */
	std::vector<DicomImageFile>::iterator iIt=images.begin();
	stackImagePos[2]=iIt->second->imagePos[2];
	int stackSliceIndex=iIt->second->stackIndex;
	float stackSlicePos=stackImagePos[2];
	bool indicesConsistent=true;
	for(++iIt;iIt!=images.end()&&indicesConsistent;++iIt)
		{
		++stackSliceIndex;
		stackSlicePos+=stackPixelSize[2];
		indicesConsistent=iIt->second->stackIndex==stackSliceIndex&&iIt->second->imagePos[2]==stackSlicePos;
		}
	if(!indicesConsistent)
		{
		/* We have a broken DICOM file. Let's just do our best: */
		std::cout<<"DicomFile::readImageStackDescriptor: Image stack has inconsistent slice indices or image slice positions"<<std::endl;
		#if 0
		DicomImageFileSorter difs(DicomImageFileSorter::ByName);
		std::sort(images.begin(),images.end(),difs);
		stackImagePos[2]=images.front().second->imagePos[2];
		#endif
		}
	
	/* Count the total number of slices, assuming that missing indices mean images were lost or corrupted: */
	int firstSliceIndex=images.front().second->stackIndex;
	int lastSliceIndex=images.back().second->stackIndex;
	int numSlices=lastSliceIndex-firstSliceIndex+1;
	
	/* Build the image stack descriptor and fill in missing slices by duplicating existing ones: */
	ImageStackDescriptor* result=new ImageStackDescriptor(numSlices);
	for(int i=0;i<2;++i)
		result->imageSize[i]=stackImageSize[i];
	for(int i=0;i<3;++i)
		result->stackPosition[i]=stackImagePos[i];
	result->sliceThickness=stackPixelSize[2];
	for(int i=0;i<2;++i)
		result->pixelSize[i]=stackPixelSize[i];
	
	/* Guess whether the image stack is flipped: */
	result->flipped=images.front().second->imagePos[2]<images.back().second->imagePos[2];
	
	/* Assemble the slice image file names: */
	char** imgFileNamePtr=result->imageFileNames;
	std::vector<DicomImageFile>::iterator difIt1=images.begin();
	std::vector<DicomImageFile>::iterator difIt2=difIt1+1;
	for(int sliceIndex=firstSliceIndex;sliceIndex<=lastSliceIndex;++sliceIndex,++imgFileNamePtr)
		{
		/* Check if we can move on to the next image: */
		if(difIt2!=images.end()&&difIt2->second->stackIndex<=sliceIndex)
			{
			/* Delete the image descriptor: */
			delete difIt1->second;
		...
		name.push_back('/');
		name.append(difIt1->first);
		
		/* Store the slice file name in the image stack descriptor: */
		*imgFileNamePtr=new char[name.length()+1];
		memcpy(*imgFileNamePtr,name.data(),name.length());
		(*imgFileNamePtr)[name.length()]='\0';
		}
	
	/* Delete the remaining image descriptor: */
	delete difIt1->second;
...
void DicomFile::readImage(const DicomFile::ImageDescriptor& id,DestPixelTypeParam* imageBuffer,const ptrdiff_t imageBufferStrides[2])
	...
	if(imageMode==IMAGE_RAW)
		...
		{
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Runlength-encoded images currently not supported");
		}
	else if(imageMode==IMAGE_JPEG_LOSSY)
		{
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Lossy JPEG images currently not supported");
		}
		...
		if(dc.readScanHeader())
			...
			if(id.imageSize[0]!=dc.getImageSize(0)||id.imageSize[1]!=dc.getImageSize(1)||dc.getNumComponents()!=1||id.pixelBitsMSB!=dc.getNumBits()-1)
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"JPEG image stream incompatible with image descriptor");
...
DicomFile::Directory* DicomFile::readDirectory(void)
	...
	if(fileType!=FILE_DIRECTORY)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"File is not a directory file");
	...
	while(true)
		...
		if(de.is(0x0004U,0x1200U)) // First directory entry offset
			...
			if(strcmp(value,"PATIENT ")==0)
				...
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unknown directory record type \"%s\"",value);
			delete[] value;
			...
			if(dlIt.isFinished())
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unlinked directory entry");
			const std::pair<Directory*,int>& dl=dlIt->getDest();
			if(dl.second==0)
				{
				if(dl.first->firstChild!=0)
					throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Doubly-linked directory entry");
