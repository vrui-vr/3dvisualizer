...
Visualization::Abstract::DataSet* StructuredGridVTK::load(const std::vector<std::string>& args,Cluster::MulticastPipe* pipe) const
	...
	/* Open the input file: */
	IO::FilePtr file(openFile(args[0]));
	...
	if(dataSetType!="STRUCTURED_GRID")
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s has data set type %s instead of STRUCTURED_GRID",args[0].c_str(),dataSetType.c_str());
	if(headerSource.readChar()!='\n')
	...
	if(headerSource.readString()!="DIMENSIONS")
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s does not define data set dimensions",args[0].c_str());
	for(int i=0;i<3;++i)
		{
		if(headerSource.peekc()=='\n')
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s has too few data set dimensions",args[0].c_str());
		numVertices[i]=headerSource.readInteger();
		}
	if(headerSource.readChar()!='\n')
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s has malformed data set dimensions",args[0].c_str());
	if(numVertices[0]<1||numVertices[1]<1||numVertices[2]<1)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s has invalid data set dimensions",args[0].c_str());
	...
	if(headerSource.readInteger()!=numVertices.calcIncrement(-1))
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"VTK data file %s defines wrong number of grid points",args[0].c_str());
