...
void JPEGDecompressor::processDht(void)
	...
	while(length>0)
		...
		if(index<0||index>=4)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Huffman table index out of range");
		...
		if(numHuffmanValues>256)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Too many values in Huffman table");
...
void JPEGDecompressor::processDri(void)
	...
	if(readShort(source)!=4)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"DRI marker has wrong length");
...
void JPEGDecompressor::processSof(int sofMarker)
	...
	if(imageSize[0]<=0||imageSize[1]<=0||numComponents<=0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Emtpy JPEG stream");
	if(numBits<2||numBits>16)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Unsupported number of bits per pixel component");
	if(length!=numComponents*3+6)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Wrong SOF marker length");
...
void JPEGDecompressor::processSos(void)
	...
	if(numScanComponents<1||numScanComponents>4)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Wrong number of components in scan");
	...
	for(int i=0;i<numScanComponents;++i)
		...
		if(componentIndex>=numComponents)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid component ID in scan");
	...
	ss=source.getChar();
	if(ss>7)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Invalid predictor type");
	source.getChar();
...
int JPEGDecompressor::processTables(void)
	...
	while(true)
		...
		switch(marker)
			...
			case DQT:
				throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Input stream is not a lossless JPEG stream");
				break;
...
void JPEGDecompressor::processRestart(short nextMarkerNumber)
	...
	if(c!=RST0+nextMarkerNumber)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Wrong number in restart marker");
	}
...
JPEGDecompressor::JPEGDecompressor(IO::File& sSource)
	...
	if(c1!=0xff||c2!=SOI)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Input stream is not a JPEG stream");
...
void JPEGDecompressor::readImage(JPEGImageWriter& imageWriter)
	...
	for(int i=0;i<numScanComponents;++i)
		if(scanComponents[i]->samplingFactors[0]!=1||scanComponents[i]->samplingFactors[1]!=1)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Downsampling not supported");
	
	/* Check that all required Huffman tables are present: */
	for(int i=0;i<numScanComponents;++i)
		if(huffmanTables[scanComponents[i]->huffmanTableIndex]==0)
			throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Undefined Huffman table used in scan");
