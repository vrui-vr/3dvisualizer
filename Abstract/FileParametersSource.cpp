...
FileParametersSource::FileParametersSource(VariableManager* sVariableManager,IO::ValueSource& sSource)
	...
	if(!source.isLiteral('{'))
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing opening brace in input file");
	...
	if(!source.isLiteral('}'))
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"Missing closing brace in input file");
	}
