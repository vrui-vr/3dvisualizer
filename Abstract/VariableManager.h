...
class VariableManager:public GLObject
	...
	struct VectorVariable // Structure containing state of a vector variable
		{
		/* Elements: */
		public:
		VectorExtractor* vectorExtractor; // Vector extractor for the vector variable
	...
	void bindColorMap(int scalarVariableIndex,SceneGraph::GLRenderState& renderState) const; // Binds the given scalar variable's color map as a 1D texture in the given OpenGL render state
	};
