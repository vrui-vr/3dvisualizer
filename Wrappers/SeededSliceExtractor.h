...
class SeededSliceExtractor:public Visualization::Abstract::Algorithm
	...
	class Parameters:public Visualization::Abstract::Parameters // Class to store extraction parameters for seeded slices
		...
		virtual void write(void* parameters) const;
		virtual void read(const void* parameters,Visualization::Abstract::VariableManager* variableManager);
		#endif
	...
	#if VISUALIZATION_CONFIG_USE_COLLABORATION
	static Collab::DataType::TypeID createClassParametersType(Collab::DataType& dataType); // Defines the algorithm's shared parameters data type
	#endif
