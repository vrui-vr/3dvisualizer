...
class Raycaster:public GLObject
	...
	typedef Misc::Size<2> Size2; // Type for 2D image sizes
	typedef Misc::Size<3> Size3; // Type for 3D image sizes
	...
	struct DataItem:public GLObject::DataItem // Structure containing per-context state
		...
		Size3 textureSize; // Size of textures able to hold the volume data
		Box texCoords; // Domain of texture coordinates to access the volume data
		Scalar mcScale[3],mcOffset[3]; // Scale factors and offsets from model space to data space
		...
		Size2 depthTextureSize; // Current size of the depth texture
		...
		/* Methods: */
		virtual void initDepthBuffer(const Size2& maxFrameSize,SceneGraph::GLRenderState& renderState); // Creates the depth texture and framebuffer based on the given maximum framebuffer size and copies the current depth buffer contents
		};
	
	/* Elements: */
	protected:
	Size3 dataSize; // Size of volume data
	...
	virtual void bindShader(const PTransform& pmv,const PTransform& mv,SceneGraph::GLRenderState& renderState,DataItem* dataItem) const; // Prepares the GLSL raycasting shader for rendering
	virtual void unbindShader(SceneGraph::GLRenderState& renderState,DataItem* dataItem) const; // Unbinds the GLSL raycasting shader after rendering
	Polyhedron<Scalar>* clipDomain(const PTransform& mv,SceneGraph::GLRenderState& renderState) const; // Clips the domain against the view frustum and all clipping planes and returns the resulting polyhedron
	
	/* Constructors and destructors: */
	public:
	Raycaster(const Size3& sDataSize,const Box& sDomain); // Creates a raycaster for the given data and domain sizes
	virtual ~Raycaster(void); // Destroys the raycaster
	
	/* New methods: */
	const Size3& getDataSize(void) const // Returns the raycaster's data size
		{
		return dataSize;
		}
	...
	virtual void glRenderAction(SceneGraph::GLRenderState& renderState) const; // Renders the data using current settings
	};
